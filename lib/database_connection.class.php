...
class DatabaseConnection {
  ...
  protected function error_test($function,$query) {
	...
	public function insertArray ($table, $data) {
		...
		foreach ($data as $k => $v) {
			if (is_null($v)) {
				...
				$newdata['`' . $k . '`'] = '"' . $this->escape($v) . '"';
			}
		}
		$data = $newdata;
		$valstr = ' (' . implode(',', array_keys($data)) . ') VALUES(' . implode(',',$data) . ')';
		$query = 'INSERT INTO ' . $table . $valstr;
		mysqli_query($this->dbh,$query);
		$this->error_test('insertArray',$query);
		return mysqli_insert_id($this->dbh);
	}
  public function updateArray ($table, $set, $where) {
		if (is_array($set)) {
			$s = array();
			foreach ($set as $k => $v) {
				if (is_null($v)) {
		...
		mysqli_query($this->dbh,$query);
		$this->error_test('updateArray', $query);
	}

  public function deleteArray ($table, $where) {
		if (is_array($where)) {
			$w = array();
			foreach ($where as $k => $v) {
				$w[] = '`'.$k.'`' . ' = "' . $this->escape($v) . '"';
		...
		mysqli_query($this->dbh,$query);
		$this->error_test('deleteArray', $query);
  }
}
